...
foreach my $source (keys %Data){
	...
	my @ERanks = @{$Data{$source}{'e'}};
	my @CRanks = @{$Data{$source}{'c'}};
	my $cmean=Tools->mean(@CRanks);
	my $emean=Tools->mean(@ERanks);
	my $wilcox_test=Statistics::Test::WilcoxonRankSum->new();
	$wilcox_test->load_data(\@CRanks,\@ERanks);
	my $ret=$wilcox_test->probability();
	my $nLogp = -1*(log($ret)/log(10));
#	print $source."\t".join(",",@CRanks)."\t".join(",",@ERanks)."\t".$ret."\n";
	#for(my $I=0;$I<scalar(@{$Data{$source}{'e'}});$I++){
		next unless $ret<0.05;
		$n++;
		next unless $cmean < $emean;
		print ">$source\n".$P{$source}."\n";
#	}
}
