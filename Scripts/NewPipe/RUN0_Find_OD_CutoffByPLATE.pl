...
sub _pickCutoff {
	...
	foreach my $ID (keys %data){
		...
		my $max=0;
		my $N = 0;
		foreach my $od (sort {$a <=> $b} keys %{$data{$ID}}){
			push @counts, $data{$ID}{$od};
			$N+=$data{$ID}{$od};
			if($data{$ID}{$od} > $max){
				$index = $od;
				$max=$data{$ID}{$od};
		...
		my $CUT = $max*$rat;
		warn "Found maximum number of ODs ($max) at od $index\nCutoff is $CUT\n";
		my $lcut = undef;
		my $ucut = undef;
		my $numcut=0;
		for(my $b=$index;$b>=0;$b-=0.01){
			my $B = sprintf("%.2f",$b);
			next unless defined $data{$ID}{$B};
			if($data{$ID}{$B}<$CUT){
				if(defined($lcut)){
					$numcut+=$data{$ID}{$B};
				}else{
		...
		print $ID." = $lcut,$ucut\n";
		warn "for $phrase, $ID\n";
		warn "Found lower cut level @ $lcut\n";
		warn "Found upper cut level @ $ucut\n";
		warn "Cut out $numcut of $N wells ($perc\%)\n\n";
